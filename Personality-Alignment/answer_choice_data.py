...
GENERATION_MESSAGE_V2 = [
    {
        "role": "system",
        "content": "You are an answer agent for multiple-choice questions. Your only task is to output one letter 'A' or 'B' inside /choice{{}} tags. Do not provide reasoning or extra text.",
    },
    {
        "role": "user",
        "content": "Choose 'A' or 'B' for the following question: {question}\n{reference_data}\n\nYour response must be exactly in the format: /choice{{X}} where X is either A or B.",
    },
]
...
def prepare_prompts_for_distractor_type(data, distractor_type, tokenizer):
    ...
    for item in data:
        ...
        # Apply chat template
        messages = [
            GENERATION_MESSAGE_V2[0],  # system message
            {
                "role": "user",
                "content": GENERATION_MESSAGE_V2[1]["content"].format(question=question, reference_data=""),
            },
        ]
...
def parse_args():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/project/hdtaccuracy/Personality-Alignment/choice_ver/v9/raw_choice_data_v9_hard_merged.jsonl",
        help="Path to input data file",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="/project/hdtaccuracy/Personality-Alignment/choice_ver/v9/evaluation_detailed_results.jsonl",
        help="Path to output detailed results file",
    )
    parser.add_argument(
        "--model_path", type=str, default="/project/hdtaccuracy/models/base/Qwen3-4B", help="Path to model"
    )
