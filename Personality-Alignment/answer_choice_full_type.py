...
PROMPT_TEMPLATES = {
    "no_think": [
        {
            "role": "system",
            "content": "You are an answer agent for multiple-choice questions. Your task is to output the correct choice in the format /choice{letter}. Do not explain, do not copy the option, do not output anything about conversation. Just output /choice{letter} where letter is A, B, C, D, E, or F.",
        },
        {
            "role": "user",
            "content": "Choose the correct answer for the following question: {question}\n{reference_data}\nYour output should be in the format /choice{{letter}} where letter is the correct choice (A, B, C, D, E, or F). Do not explain, do not copy the option, do not output anything about conversation.\nNow, your output is:",
        },
    ],
    "with_think": [
        {
...
def extract_choice_from_response(response):
    ...
    # 2. 查找其他可能的格式变体
    patterns = [
        r"choice\s*[:\-]?\s*([A-F])",  # choice: A 或 choice A
        r"answer\s*[:\-]?\s*([A-F])",  # answer: A 或 answer A
        r"option\s*[:\-]?\s*([A-F])",  # option: A 或 option A
        r"^([A-F])[.\s]",  # 开头的单个字母
        r"\b([A-F])\b",  # 任何单独的字母
    ]
...
def prepare_prompts_for_evaluation(questions, prompts_dict, tokenizer=None, backend="hf", prompt_type="no_think"):
    ...
    # 选择使用的prompt模板
    template = PROMPT_TEMPLATES.get(prompt_type, PROMPT_TEMPLATES["no_think"])
    ...
    for question in questions:
        ...
        messages = [
            template[0],  # system message
            {
                "role": "user",
                "content": template[1]["content"].format(question=question_text, reference_data=""),
            },
        ]
        ...
        for attempt in range(3):
            try:
                resp = client.chat.completions.create(
                    ...
                    messages=messages,
                    temperature=0,
                    max_tokens=50,  # 增加tokens以支持thinking过程
                ...
                # 使用新的choice提取函数
                choice = extract_choice_from_response(response)
                return choice
    ...
    for i in tqdm(range(0, len(prompts), batch_size), desc="Processing batches"):
        ...
        with torch.no_grad():
            outputs = model.generate(
                **input_tokens,
                max_new_tokens=200,  # 增加tokens以支持thinking过程
                do_sample=False,
        ...
        for j, output in enumerate(outputs):
            ...
            if choice is not None:
                ...
                tqdm.write(f"Invalid response: {response[:100]}...", end="\n")
                continue
...
def main():
    ...
    parser.add_argument(
        "--prompt_type", choices=["no_think", "with_think"], default="no_think", help="Type of prompt template to use"
    )
    ...
    # Prepare prompts
    print(f"Preparing prompts with template: {args.prompt_type}...")
    prompts, correct_choices, question_ids = prepare_prompts_for_evaluation(
        questions, prompts_dict, tokenizer=tokenizer, backend=args.backend, prompt_type=args.prompt_type
    )
